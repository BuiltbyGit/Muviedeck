...
$stmt = $conn->prepare('TRUNCATE TABLE getpopularseriesid');
$stmt->execute();
...
curl_setopt_array($curl, [
	CURLOPT_URL => "https://imdb8.p.rapidapi.com/title/get-most-popular-tv-shows?homeCountry=US&purchaseCountry=US&currentCountry=US",
...
for ($i=0; $i < 50; $i++) { 
	$getTitleexplode = explode("/", $response[$i]);
	echo $getTitleexplode[2];
	echo '<br/>';
	$stmt = $conn->prepare('INSERT INTO getpopularseriesid (title) VALUES (?)');
