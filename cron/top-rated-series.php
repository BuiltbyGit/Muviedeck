...
$stmt = $conn->prepare('TRUNCATE TABLE gettopratedseriesid');
$stmt->execute();
...
curl_setopt_array($curl, [
	CURLOPT_URL => "https://imdb8.p.rapidapi.com/title/get-top-rated-tv-shows",
...
for ($i=0; $i < 50; $i++) { 
	$getTitleexplode = explode("/", $response[$i]['id']);
	echo $getTitleexplode[2];
	echo '<br/>';
	$stmt = $conn->prepare('INSERT INTO gettopratedseriesid (title) VALUES (?)');
