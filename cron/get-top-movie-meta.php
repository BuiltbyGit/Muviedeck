...
$response = json_decode($response,true);
	...
	$stmt = $conn->prepare('INSERT INTO topmoviemeta (MovieId, MovieTitle, MovieRating, MovieRunTime, MoviePlot, MoviePoster, MovieReleaseDate, MovieCast, MovieLanguage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
	...
	foreach ($movieCast as $key => $value) {
				unset($castResult);
	$castResult[] = $key .' - '.$value['actorBirthName'].', ';
	$movieMainCast = $castResult[0];
	}
	
 $stmt = $conn->prepare('INSERT INTO topratedmoviesmeta (MovieId, MovieTitle, MovieRating, MovieRunTime, MoviePlot, MoviePoster, MovieReleaseDate, MovieCast, MovieLanguage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
 $stmt->bind_param('sssssssss', $movieId, $movieTitle, $movieRating, $movieRuntime, $moviePlot, $moviePoster, $movieReleaseDate, $movieMainCast, $movieLanguage);
 $stmt->execute();
 $stmt->close();